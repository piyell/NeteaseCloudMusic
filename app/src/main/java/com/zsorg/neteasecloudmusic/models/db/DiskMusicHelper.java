...
public class DiskMusicHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("create table if not exists playlist(list_id integer not null  default 1 primary key autoincrement,name text,path text);");
        db.execSQL("insert into playlist(list_id,name) values(0,\"我喜欢的音乐\");");
        db.execSQL("create table if not exists playlist_detail(list_id integer not null,path varchar not null);");
    }
